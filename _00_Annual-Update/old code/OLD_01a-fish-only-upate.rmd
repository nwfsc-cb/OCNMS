...
setwd("~/GitHub/OCNMS/_00_Annual-Update")
...
esr_recruits = fish_taxa_year[,c('year','taxa','grand_mean','grand_SE')]
esr_recruits = esr_recruits %>% rename(index = grand_mean, SE = grand_SE) %>%
                        mutate(metric = "Number per 120 m^3") %>%
                        filter(taxa %in% fish.taxa$taxa)
esr_recruits$timeseries = fish.taxa$name[match(esr_recruits$taxa, fish.taxa$taxa)]
write.csv(esr_fish,"GF-ocnms-rockfish-yoy.csv", row.names = F)
...
```{r GF-black-rockfish, height=1.5, width = 3.2}
fish.col = c(RColorBrewer::brewer.pal(n = 12, name = "Paired"))

fish.plot <- ggplot(fish_taxa_year %>% filter(taxa == 'SEME') %>% left_join(.,fish.taxa)) +
  geom_point(aes(x=year,y=grand_mean)) + 
  geom_line(aes(x=year,y=grand_mean)) +
...
esr_fish = fish_taxa_year[,c('year','taxa','grand_mean','grand_SE')]
esr_fish = esr_fish %>% rename(index = grand_mean, SE = grand_SE) %>%
                        mutate(metric = "Number per 120 m^3") %>%
                        filter(taxa %in% fish.taxa$taxa)
esr_fish$timeseries = fish.taxa$name[match(esr_fish$taxa, fish.taxa$taxa)]
write.csv(esr_fish,"GF-ocnms_fish_adults.csv", row.names = F)
