...
run.multivar.nt <- function(data.file, drop2015 = TRUE, spp, 
                            data.transform = NA, both_zones = TRUE, nperm = 999, 
                            outname = 'Results-' ){
     ...
     # library(ecole) in phytomosaic package
     df_matrix <-vegan::vegdist(df, method = 'manhattan')
     
     assign('df_matrix', get('df_matrix'), pos=1)
     
     cap = CAPdiscrim( df_matrix ~ as.factor(CAPid) , data=CAPfile , m=0 , 
                       permutations=nperm)
