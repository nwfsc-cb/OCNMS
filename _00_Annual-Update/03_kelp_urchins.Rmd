...
df_transect1 = readRDS(paste0(data_dir,'Data_Inverts_Kelp_transect_wide.rds')) %>% 
...
```{r kelps-v-allurchins-facet, fig.height=5.5, fig.width=3.5}
...
```{r kelp-urchins-by-site-lms , fig.width=6, fig.height=7}
df = df_transect %>% group_by(site, year, zone, area, transect, kelp_spp) %>%
                      summarise(urchins = mean(urchin_density), kelp = mean(kelp_density) ) %>% 
                      group_by(site,year,kelp_spp) %>%
                      summarise(Urchins = mean(urchins), Kelp = mean(kelp) )

kelps <- as.character(levels(as.factor(df$kelp_spp)))


for(i in 1:5){
  dfs = df %>% filter(site == sites[i])
  for(k in 1:4){
    dfx = dfs %>% filter(kelp_spp == kelps[k])
    lmx = lm(Kelp+0.1 ~ Urchins, data = dfx)
    smx = summary(lmx)
    b0 = paste0(round( smx$coefficients[1,1],2), " (",round( smx$coefficients[1,2],2),")")
    b1 = paste0(round( smx$coefficients[2,1],2), " (",round( smx$coefficients[2,2],2),")")
    r2 =  round( smx$r.squared , 2 )
    pval = round( smx$coefficients[2,4] , 2 )
    dfk = data.frame(cbind(Site = as.character(dfx$site[i]), 
                         Species = as.character(dfx$kelp_spp[i]), 
                         b0 = b0,
                         b1 = b1,
                         R2 = r2, 
                         pval = pval))
    if(k==1){DFk = dfk}else{DFk = rbind(DFk, dfk)}
  }
  
  if(i==1){DFi = DFk}else{DFi = rbind(DFi, DFk)}
}

# DFi

write.csv(DFi, paste0(fig_dir,"Kelp-Urchin-regressions-site-x-year.csv"), row.names = FALSE)



KU <- ggplot(df , aes(x = Urchins, y = Kelp)) + geom_point() + 
      facet_grid(site ~ kelp_spp, scales = 'free') 
     
KU + theme_bw() + theme_nt
...
dfm = df %>% filter( kelp_spp == "Macrocystis" &    site == 'Neah Bay')

mx = lm(Kelp ~ Urchins, data = dfm)
smx = summary(mx)
#
r2 =  round( smx$r.squared , 2 )
pval = round( smx$coefficients[2,4] , 2 )

rstatement = bquote(italic(r)^2 == .(format( smx$r.squared, digits = 2)))
...
```{r neah-year, fig.height=2.5, fig.width=3.5}
...
# macro
mx = lm(Kelp ~ Urchins, data = dfm%>%filter(kelp_spp=="Macrocystis"))
smx = summary(mx)
#
r2 =  round( smx$r.squared , 2 )
pval = round( smx$coefficients[2,4] , 2 )
...
neah1 <- ggplot(dfn, aes( x=Urchins , y=Kelp, color=kelp_spp)) +
         ...
         ylim(0,NA)+
         # macro       
         annotate(geom="text", x=0.05, y=0.9, label = expression(italic("Macro")), hjust = 0, color=site.col$col[1], size = 3) +
         annotate(geom="text", x=0.05, y=0.5, label = rstatement.m, hjust = 0, color=site.col$col[1], size = 3) +
         annotate(geom="text", x=0.05, y=0.1, label = pstatement.m, hjust = 0, color=site.col$col[1], size = 3) +
         # Ptery
         annotate(geom="text", x=0.0, y=3.00, label = expression(italic("Ptery")), hjust = 0, color=site.col$col[3], size = 3) +
         annotate(geom="text", x=0.0, y=2.6, label = rstatement.p, hjust = 0, color=site.col$col[3], size = 3) +
         annotate(geom="text", x=0.0, y=2.2, label = pstatement.p, hjust = 0, color=site.col$col[3], size = 3) +
         xlab( expression(paste('Urchins per ', m^2) ) )+
...
```{r other-vs-urchins-site-year, fig.height=2.5, fig.width=3.5}
...
other = filter(df, kelp_spp == "Other")
...
other1 <- ggplot(other, aes( x=Urchins , y=Kelp, color=site)) +
         geom_point(size=2.5, pch=other$pch) +
         scale_color_manual(values=site.col$col )+
         xlim(0,NA)+
         ylim(0,NA)+
         ...
         #          size = 3) +
         xlab( expression(paste('Urchins per ', m^2) ) )+
         ylab( expression(paste('Other', ' stipes per ', m^2)) )+
         theme_bw() + theme_nt 
...
pl <- df_transect %>%
  ...
  scale_color_manual(values=site.col$col )+
  facet_grid(kelp_spp ~ urchin_spp)+
  theme_bw()+ theme_nt + 
...
```{r tatoosh-transect-level-other , fig.height=2.5 , fig.width=3.5}

dfx <-df_transect %>% filter(., site == 'Tatoosh Island') %>% filter(kelp_spp == 'Other') %>%
                      group_by(site,year,transect,area,zone) %>%
                      summarize( kelp = mean(kelp_density) , 
                                 urchins = mean(urchin_density) )
dfx$pch = as.character(substring(dfx$year,4,4))
...
```{r Figure-5-prep, fig.height=6.65, fig.width=6.65}
...
# c
dfx = na.omit(df %>% filter(., group == "Macrocystis"))
X = dfx$Urchins
Y = dfx$Kelp
R = cor.test(X,Y)
...
# Nereo
dfx = na.omit(df %>% filter(., group == "Nereocystis"))
X = dfx$Urchins
Y = dfx$Kelp
R = cor.test(X,Y)
...
# other
dfx = na.omit(df %>% filter(., group == "Other"))
X = dfx$Urchins
Y = dfx$Kelp
R = cor.test(X,Y)
...
plot1 <- ggplot( df%>%filter(df$group %in% c('Macrocystis','Nereocystis','Pterygophora','Other')) ,
  ...
  xlab( expression(paste('Urchins per ', m^2) ) )+
  ylab( expression(paste('Stipes per ', m^2) ) )+
  geom_smooth(data = filter(df,df$group =='Pterygophora' ),method='lm', se=FALSE) +
  annotate(geom="text", x=stats.x, y=2.0, label = expression(italic("Pterygophora")), hjust = 0, color=site.col$col[3], size = 3) +
  annotate(geom="text", x=stats.x, y=1.7, label = rstatement3, hjust = 0, color=site.col$col[3], size = 3) +
  annotate(geom="text", x=stats.x, y=1.4, label = pstatement3, hjust = 0, color=site.col$col[3], size = 3) +
          theme_bw() + theme_nt 
...
```{r Figure-5, fig.height=7.2, fig.width=6.65}
...
other2 <- other1 + theme(legend.position = 'none')+  ylim(0,3.5) +
                         annotate(geom="text",x=0.05,y=3.4, label="e)")
neah2 <- neah1 +   theme(legend.position = "none") + ylim(0,3.5) +
                         annotate(geom="text",x=0.005,y=3.4, label="f)")
nereo.tat2 <- nereo.tat +theme(legend.position = c(0.75,0.4))+ ylim(0,15)+xlim(0,8)+
                         annotate(geom="text",x=0.5,y=14.5, label="g)")
ptery.tat2 <- ptery.tat +theme(legend.position = 'none')+ ylim(0,15)+xlim(0,8)+
                         annotate(geom="text",x=0.5,y=14.5, label="h)")
other.tat2 <- other.tat +theme(legend.position = 'none')+ ylim(0,15)+xlim(0,8)+
                         annotate(geom="text",x=0.5,y=14.5, label="i)")
