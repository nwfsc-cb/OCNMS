...
data_dir = "~/GitHub/OCNMS/Data/2024/"
...
fishx = data.frame(read_xlsx(
  paste0(data_dir,"NWFSC_FISH_ALLYEARS_data_2024.xlsx"), sheet='DATA'))
fish_codes = data.frame(read_xlsx(
  paste0(data_dir,"NWFSC_FISH_ALLYEARS_data_2024.xlsx"), sheet='species.codes'))

# inverts and algae 
swathx = data.frame(read_xlsx(
  paste0(data_dir,"NWFSC_SWATH_ALLYEARS_data_2024.xlsx"), sheet='data'))
swath_codes = data.frame(read_xlsx(
  paste0(data_dir,"NWFSC_SWATH_ALLYEARS_data_2024.xlsx"), sheet='species.codes'))
swath_codes = swath_codes %>% rename(species = CLASSCODE) %>%
  mutate(group = stringr::str_to_sentence(group))
source("_02_swath-inverts-update.R")

```
# Data import

Currently the species codes are in separate files from the data. This set up may change. 

```{r data-import}
# fish
fish0 = readRDS( paste0(data_dir,'Fish_2015-2024.rds' ))
# algae and kelp
swath0 <- readRDS( paste0(data_dir,'Swath_2015-2024.rds' ))
...
# fish_jackie = fish0 %>% filter(year == 2015, site %in% c("Teahwhit Head", "Rock 305"))
# swath_jackie = swath0 %>% filter(year == 2015, site %in% c("Teahwhit Head", "Rock 305"))
# write.csv(fish_jackie, "Fish-transects-2015-TH.R305.csv")
# write.csv(swath_jackie, "Invers-algae-transects-2015-TH.R305.csv")
...
I set some common settings here. The settings.rds file ports the same info over to the multivariate rmd. It is brough in and udated here. 
...
Table is count of pynopodia by year. 
...
canopy = readRDS( paste0(data_dir, "Data_Kelp_Canopy_Long.rds") )
...
df <- data.frame(read_excel( paste0(data_dir,
...
att <- data.frame(read_excel( paste0(data_dir,'Mean Daily OISST (C) for OCNMS sites 2003 to 2021.xlsx'), 
...
plot5 <- ggplot(df_max_month, aes(x = year, y = maxT) ) +
    ...
    scale_x_continuous( breaks = seq(2005,2020,5), minor_breaks = 2003:last.year )
