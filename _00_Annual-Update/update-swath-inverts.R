...
# dat.2016.on.swath <- read.csv("NWFSC_SWATH_ALLYEARS_data_2024.csv")
...
nom <- c("Destruction Island","Teahwhit Head",
         "Cape Johnson","Rock 305","Cape Alava",
         "Point of the Arches",
         "Anderson Point","Tatoosh Island",
...
dat.2015 <- left_join( dat.2015, species_names %>% 
                         dplyr::select(species,group) %>% 
                         rename(PISCO.Classcode=species))



base.dat <- dat.2015 %>% 
  filter(PISCO.datatype=="swath",data.type=="swath") %>% 
  group_by(Site,Transect,Observer,Transect.area..inverts.,group) %>% 
  summarise(x=length(Transect)) %>% filter(is.na(Transect)==F) %>% 
  dplyr::select(Site,Transect,Observer,Transect.area=Transect.area..inverts.,group) %>% as.data.frame()
...
SP.all <- data.frame(species=unique(c(as.character(swath.dat$PISCO.Classcode),
                                      as.character(dat.2016.on.swath$CLASSCODE))))
...
# manually remove an oddball observation from Jameal, 2015, destruction Is., 
# for the three main canopy species.
dat.long <- dat.long %>% 
     ...
     mutate(Transect.area = 
           ...
           # remove non observed transects, eg -99           
           filter(!Transect.area==-99)
                                 
dat.2015.swath  <-  dat.long
dat.2015.swath.count <- dat.long %>% 
  group_by(site,transect,observer,Transect.area,common.name,species,year,group) %>%
...
base.dat <- dat.2016.on.swath %>%
   ...
   summarise(x=length(TRANSECT)) %>% dplyr::select(-x) %>%
   as.data.frame()

swath.dat <- dat.2016.on.swath %>% 
  group_by(YEAR,SITE,AREA,TRANSECT,OBSERVER,ZONE,CLASSCODE,SIZE,SEGMENT,group,METERS.sampled) %>% 
  # drop non observed rows with -99 designation ####
  filter(COUNT !=-99) %>%
  summarise(Count=sum(COUNT)) %>% 
  rename(SPECIES=CLASSCODE) %>% 
  as.data.frame()
...
dat.2016.plus.swath = dat.2016.plus.swath %>% 
                      mutate(expand = 10 / METERS.sampled,
...
dat.2016.plus.swath <- dat.2016.plus.swath %>% 
  group_by(year,site,area,transect,observer,zone,species,group,SEGMENT) %>%
  summarize(Count.seg=sum(total.count)) %>% 
...
#       as.data.frame()
# 
 dat.swath.base %>% ungroup() %>% 
   dplyr::select(year, site,area,transect) %>% 
   distinct(year,site,area) %>%
   as.data.frame()
...
dat.transect.summary <- dat.swath.base %>% filter(year>2015) %>% 
  group_by(year,site,area,zone,group) %>%
...
write.csv(dat.transect.summary,file=paste0(data_dir,"SWATH ERROR CHECK.csv"),row.names=F)
