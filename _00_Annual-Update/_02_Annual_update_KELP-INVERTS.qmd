---
title: "Annual Update - KELP and INVERTS"
...
To update the data to the most recent year, you will need:
...
# output file names
# swath_out = paste0(data_dir,'NWFSC_SWATH_ALLYEARS_data_',data_year,'.xlsm')

# download
# drive_download( as_id(swath_id) , path = swath_out , overwrite = TRUE)

# convert to csv by hand. the xlsx has loading problems ########################
# size column goes to T/F for some reason

swathxl = data.frame(read.csv(paste0(data_dir,'NWFSC_SWATH_ALLYEARS_data_',data_year,'.csv')))
...
The 'settings.R' files has triggers and codes etc. 
...
# sites[sites="Destruction Island SW"] <- "Destruction Island"
...
################################################################################
# KELP STIPES BY TRANSECT ######################################################
################################################################################
algae1 <- dat.algae %>% group_by(year,site,transect,zone,area,fun_gr,species) %>%
...
saveRDS(algae1, file = paste0(data_dir,"Summarized_Data_Kelp_by_Transect.rds"))
write.csv(algae1, paste0(data_dir,"Summarized_Data_Kelp_by_Transect.csv"))
################################################################################
################################################################################
################################################################################
...
write.csv(df_out, "~/GitHub/NMS-condition-reports/data/ocnms-dive-kelp.csv", row.names = F)
...
# write out to OCNMS condition report
write.csv(df_out, "~/GitHub/NMS-condition-reports/data/ocnms-dive-understory-kelp.csv", row.names = F)
```
...
################################################################################
# INVERSTS SUMMARIZED BY TRANSECT ##############################################
################################################################################
...
saveRDS(invert_transect, file=paste0(data_dir , "Summarized_Data_Inverts_by_Transect.rds") )
write.csv(invert_transect,paste0(data_dir , "Summarized_Data_Inverts_by_Transect.csv"), row.names = F )
...
write.csv(p_out, '~/GitHub/NMS-condition-reports/data/ocnms-dive-total-pycno.csv', row.names = F)
...
write.csv(df_out, "~/GitHub/NMS-condition-reports/data/ocnms-dive-pynopodia.csv", row.names = F)
...
ggplot(df %>% filter(species=="PYCHEL"), aes(year, mn, color=site)) +
 ...
 write.csv(df_out, "~/GitHub/NMS-condition-reports/data/ocnms-dive-pynopodia-site.csv", row.names = F)
...
### Coastwide Urchins vs kelp Differenced
...
write.csv(stars_out, "~/GitHub/NMS-condition-reports/data/ocnms-dive-seastars.csv", row.names = F)
...
canopy = readRDS( paste0(home_dir, "Data_Kelp_Canopy_Long.rds") )
...
```{r kelpwatch, message=FALSE, error=FALSE, eval=FALSE}

kw = data.frame(read.csv(paste0(repo_dir, "/Data/Kelp/kelp-west-coast.csv")))
...
```{r Figure-2, fig.width=6, fig.height=6, eval=FALSE}
