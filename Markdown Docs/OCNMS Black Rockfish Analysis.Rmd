...
```{r echo=FALSE, warning=F,message=FALSE} 
# GENERATE INDEXES BY SITE and AMONG SITES
dat.can.yoy.summary <- dat.can 

# summarize using means and standard errors
dat.can.yoy.summary <- dat.can.yoy.summary %>%
                    group_by(year,site,species) %>% 
                    summarise(Mean=mean(Count),
                              VAR = var(Count),
                              SD=sd(Count),
...
# This is a simple calculation treating the means from each site as observed and iid samples of a grand Mean
dat.can.yoy.out.A <- dat.can.yoy.summary %>%
                  group_by(year)  %>%
                  summarise(grand.mean = mean(Mean),
                            N= length(Mean),
                            SE = sd(Mean)/sqrt(N))
...
sim <- matrix(# simulate from a centered t-distribution.
        rt(nrow(dat.can.yoy.summary) * n.sim,dat.can.yoy.summary$N - 1) * dat.can.yoy.summary$SE +
                dat.can.yoy.summary$Mean, # Adjust to a different location and scale.
                nrow(dat.can.yoy.summary), n.sim) # Put it in a matrix.
sim[sim <0] <- 0
sim <- as.data.frame(sim)
colnames(sim) <- paste0("X",1:ncol(sim))

sim <- bind_cols(dat.can.yoy.summary %>% dplyr::select(year,site), sim) %>%
          pivot_longer(.,cols=3:ncol(.),values_to="val",names_to="real")

can.yoy.trend <- sim %>% group_by(year,real) %>%
          dplyr::summarise(mean.real = mean(val)) %>% ungroup() %>%
          group_by(year) %>%
          dplyr::summarise(grand.mean = mean(mean.real),
                           SE = sd(mean.real),
...
# Make canary rockfish time-series plots
can.yoy.ts <- ggplot() + 
  geom_point(data=dat.can.yoy.summary, aes(x=year,y=Mean,color=species),alpha=0.2) +
  geom_errorbar(data=dat.can.yoy.summary,
                aes(x=year,ymin=Mean-SE,ymax=Mean+SE),
                width=0,
                alpha=0.2) +
  geom_point(data=can.yoy.trend, aes(x=year,y=grand.mean),
             color="blue",size=3)  +
  geom_errorbar(data=can.yoy.trend,
                aes(x=year,ymin=q025,ymax=q975),
                width=0,
                color="blue",size=0.5) +
  geom_errorbar(data=can.yoy.trend,
