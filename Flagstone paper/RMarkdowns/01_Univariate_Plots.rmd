...
# combine at taxa level for multivariate analyses
invert_transect_taxa <- dat.invert %>% group_by(year,site,transect,observer,zone,area,taxa,grp2) %>%
    summarise(total.count=sum(Count),total.area=sum(Transect.area)) %>% 
    mutate(density = total.count / total.area )
...
invert_kelp <- invert_kelp[,c('year','site','area','zone','taxa','density')]
