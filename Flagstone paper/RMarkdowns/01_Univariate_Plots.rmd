...
```{r fish-yoy, fig.width=5, fig.height=4}
...
dfx <- big3 %>% group_by(year, species) %>%
                summarise(Mean = mean(mean.density), 
                          SD = sd(mean.density),
                          N = length(year),
                          SE = SD/sqrt(N)
                )
...
dfx5$species[dfx5$species=='NERLUE'] <- 'Nerocystis'  
dfx5$species[dfx5$species=='PTECAL'] <- 'Pteropophora'  
...
urchin_year <- urchins_all %>% group_by(taxa, year) %>%
               summarize(Mean = mean(mean.density), 
                         SD = sd(mean.density), 
                         N = length(site),
                         SE = sqrt(SD)/N)
...
urchin.plot <- ggplot(urchin_year, aes(x = year, y=Mean, color=taxa)) +
                ...
                scale_x_continuous("", breaks=seq(2015,2021)) +
                scale_y_continuous(expression("Urchin density (m"^-2*")"), limits=c(0,2)) +
                theme_bw()+
                theme(legend.title = element_blank(),
                      legend.text = element_text( size = 8),
                      axis.text.x = element_text(size=10),
...
htmlTable(fish_total, rnames=FALSE)
...
```{r Figure-2, fig.width=5, fig.height=7}

# library(ggpubr)


ggarrange(
  kelp_coast5, urchin.plot, fish.plot,yoy.plot,
  labels = c('a)','b)','c)','d)'), 
  # labels = 'auto',
  font.label = list(face='plain', size = 10),
  hjust = -5,
