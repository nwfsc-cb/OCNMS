...
---
```{r run-univariate-workhorse-rds }
# This script imports fish data, does manipulations.
# Imports Invert and Algae Data, does manipulations.
# I added some specific data imports so we don't have to run the univariate workhorse each time
...
fish_codes = settings$fish_codes
swath_codes = settings$swath_codes
...
### fix area labels for fish ####
### match to swath


fish0$area[fish0$year == 2015] <- 'W'

fish0$n = 1
x = aggregate(n ~ year + area + zone + site, data = fish0, FUN = sum)


fish0$area[fish0$site %in% c('Cape Alava') & fish0$area == 1] <- 'E'
fish0$area[fish0$site %in% c('Cape Alava')& fish0$area == 2] <- 'W'

fish0$area[fish0$site %in% c('Destruction Island',
                             'Cape Johnson') & fish0$area == 1] <- 'S'
fish0$area[fish0$site %in% c('Destruction Island',
                             'Cape Johnson') & fish0$area == 2] <- 'N'
fish0$area[fish0$site %in% c('Tatoosh Island',
                             'Neah Bay') & fish0$area == 1] <- 'N'
fish0$area[fish0$site %in% c('Tatoosh Island',
                             'Neah Bay') & fish0$area == 1] <- 'N'
# fish0 <- fish0 %>%
#   mutate(
#     area = case_when(
#       site == "Cape Alava" & area == 1 ~ "W",
#       site == "Cape Alava" & area == 2 ~ "E",
#       site == "Cape Johnson" | site == "Destruction Island" & area == 1 ~ "S",
#       site == "Cape Johnson" | site == "Destruction Island" & area == 2 ~ "N",
#       site == "Neah Bay" | site == "Tatoosh Island" & area == 1 ~ "N",
#       site == "Neah Bay" | site == "Tatoosh Island" & area == 2 ~ "N",
#       TRUE ~ NA_character_
#       )
#   )
...
# correct a names
swath0$site[swath0$site == "Destruction Island SW"] <- 'Destruction Island'

swath0$area[swath0$year == 2015] <- 'W'
swath0$n=1
x = aggregate(n ~ year + area + zone + site, data = fish0, FUN = sum)

swath0$area[swath0$site %in% c('Cape Alava') & swath0$area == 1] <- 'E'
swath0$area[swath0$site %in% c('Cape Alava')& swath0$area == 2] <- 'W'

swath0$area[swath0$site %in% c('Destruction Island',
                             'Cape Johnson') & swath0$area == 1] <- 'S'
swath0$area[swath0$site %in% c('Destruction Island',
                             'Cape Johnson') & swath0$area == 2] <- 'N'
swath0$area[swath0$site %in% c('Tatoosh Island',
                             'Neah Bay') & swath0$area == 1] <- 'N'
swath0$area[swath0$site %in% c('Tatoosh Island',
                             'Neah Bay') & swath0$area == 1] <- 'N'
# swath0 <- swath0 %>%

saveRDS(swath0, file=paste0(Data_Loc,'Swath_2015-2021-ntmods.rds'))
...
Problems here...with matching up rows. The area has different names in the fish and kelp files.

```{r fish-kelp-combine}
###### fish ####
########fish1 includes all spp and five sites but no deletion of low ivs.

fishx1 = fish1[fish1$site %in% sites,]
swathx = swath0[swath0$site %in% sites,]
# drop low vis sites/transects


# x = aggregate( vis_m ~ year+zone+area+site, data=fishx1, FUN = range)


fishx = fishx1

#fishx = fishx1[fishx1$vis_m >= min.vis, ]

# double check specific averaging process here 

fish_area = aggregate( Count ~ site + zone + year + area + taxa, data = fishx, FUN = mean )
colnames(fish_area)[ncol(fish_area)] <- 'mean'
fish_area_wide = pivot_wider(fish_area , names_from = taxa , values_from = mean, values_fill = 0)
fishid = paste(fish_area_wide$site, 
               fish_area_wide$year,
               fish_area_wide$zone,
               fish_area_wide$area, sep="_")
...
kelp_area = aggregate( Count ~ site + zone + year + area + taxa, data = swathx, FUN = mean )
colnames(kelp_area)[ncol(kelp_area)] <- 'mean'
kelp_area = kelp_area[kelp_area$taxa %in% c("MACPYR", "NERLUE", "PTECAL"),]
kelp_area_wide = pivot_wider(kelp_area , names_from = taxa , values_from = mean, values_fill = 0)
kelpid = paste(kelp_area_wide$site, 
               kelp_area_wide$year,
               kelp_area_wide$zone,
               kelp_area_wide$area, sep="_")
...
```{r kelp-vs-urchins, fig.width=3.5, fig.height==3.5}
...
>>>>>>> f92b1749a40482600ee9c46c016207f452ef2310
