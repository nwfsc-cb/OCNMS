...
```{r big3, fig.width=6, fig.height=4}
 ...
 kelp.labs = c('Macrocystis','Nerocystis','Pterogophora')
...
# join to facet later
kus <-  mac_ner_zone %>%
               full_join(., y=star_zone) %>% 
               full_join(., y=urchin_zone) %>%
...
```{r kelp-urchin-stars-zone, fig.width=5, fig.height=4}
...
kus_year_zone <- kus_year_zone %>%
                left_join(.,SE, by=c('year'='year','grp2'='grp2','zone'='zone')) %>%
                rename(SE = SE.y)
...
```{r kelp-urchin-stars-zone-5-10, fig.width=3.5, fig.height=4}
...
```{r kelp-urchin-stars-5, fig.width=3.5, fig.height=4}
...
f1 <-  fish_transect_taxa %>% group_by(site,year,area, zone, taxa) %>%
                  summarise(Mean=mean(Count),SD=sd(Count),N=length(Count), SE=SD/sqrt(N),SE_var=SE^2)

i1 <-  invert_transect_taxa %>% group_by(site,year,area, zone, taxa) %>% 
                  summarise(Mean=mean(density),SD=sd(density),N=length(density), 
                            SE=SD/sqrt(N),SE_var=SE^2)
# Output for capscale analyses
# fish-kelp joined at the area level because transects aren't the same
fish_kelp <- f1 %>% full_join(. , y=k1) %>% rename(mean= Mean)
saveRDS(fish_kelp, paste0(Data_Loc, "Data_Fish_Kelp_transect_long.rds") )
fish_kelp <- fish_kelp[,c('year','site','area','zone','taxa','mean')]
fish_kelp <- pivot_wider(fish_kelp, names_from = taxa, values_from = mean, values_fill = 0)
saveRDS(fish_kelp, paste0(Data_Loc, "Data_Fish_Kelp_transect_wide.rds") )

# combine inverts and kelp at transect level
kelp_transect$taxa <- kelp_transect$species # create categroy
invert_kelp <- kelp_transect %>% full_join(., y=invert_transect_taxa)
saveRDS(invert_kelp, paste0(Data_Loc,"Data_Invert_Kelp_transect_long.rds") )

invert_kelp <- invert_kelp[,c('year','site','area','transect','zone','taxa','density')]

invert_kelp$n = 1
x = aggregate( n ~ year + site + area + transect + zone +taxa, data = invert_kelp , FUN = sum)



invert_kelp <- na.omit(invert_kelp)
invert_kelp$taxa <- str_replace(invert_kelp$taxa, " ","_")
invert_kelp <- pivot_wider(invert_kelp, names_from = taxa, values_from = density, values_fill = 0)
saveRDS(invert_kelp, paste0(Data_Loc,"Data_Invert_Kelp_transect_wide.rds") )

# output for CAPdiscrim analyses ####

fish_wide = pivot_wider(fish_transect_taxa[,c('year','site','transect','area','zone','Count','taxa')] ,
                        values_from = Count, names_from = taxa, values_fill = 0 )
saveRDS(fish_wide, paste0(Data_Loc,  "Data_FISH_taxa_wide") )

kelp_wide = pivot_wider(kelp_transect[,c('year','site','transect','area','zone','taxa','density')] ,
                        values_from = density, names_from = taxa, values_fill = 0 )
saveRDS(kelp_wide, paste0(Data_Loc,   "Data_KELP_wide") )

invert_wide = pivot_wider(invert_transect_taxa[,c('year','site','transect','area','zone','density','taxa')] ,
                        values_from = density, names_from = taxa, values_fill = 0 )
saveRDS(invert_transect_taxa, paste0(Data_Loc,"Data_INVERT_wide.rds") )
