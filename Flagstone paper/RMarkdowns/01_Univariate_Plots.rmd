...
# x = invert_transect %>% filter(grp2=="URCHIN") %>%
#     # sum urchin density by transect
#     group_by(year,site,transect,observer,zone,area,taxa) %>%
#     summarise(tot.density = sum(density, na.rm = TRUE)) %>%
#     # group by year, site, taxa means
#     group_by(year,site,taxa) %>%
#     summarise(mean.density=mean(tot.density, na.rm = TRUE)) %>%
#     # get year means by urchin spp
#     group_by(taxa, year) %>%
...
canopy.kelp <- ggplot( canopy %>% filter(., site == 'Coastwide') %>% filter(., year %in% 1990:2021), 
...
# library(ggpubr)
canopy.kelp <- canopy.kelp + theme(legend.position = c(0.2 , 0.7) )
kelp_coast <- kelp_coast + theme(legend.position = c(0.30 , 0.9) )
