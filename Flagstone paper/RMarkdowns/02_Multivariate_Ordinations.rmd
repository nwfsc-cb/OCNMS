...
fish <- fish %>% rename('lingcod' = 'OPEL') %>%
                 ...
                 rename('cabazon' = 'SCMA') %>%
                 rename('blue.rockfish' = 'SENE') %>%
                 rename('black.rockfish' = 'SEME')
fish.spp = c('lingcod', 'greenlings','copper.rockfish','cabazon','blue.rockfish','black.rockfish')



kelp <- kelp %>% rename('Macrocystis' = 'MACPYR') %>%
                 rename('Nerocystis' = 'NERLUE') %>%
                 rename('Pterogophora' = 'PTECAL')
kelp.spp = c('Macrocystis','Nerocystis','Pterogophora')
...
# fish.spp =   c("OPEL", "HEXA", "SECA", "SCMA" ,"SENE", "SEME")

yoy.spp =    c("SECAyoy", "SEPIyoy", "SEMYyoy", "SEBYTyoy","RYOY")


# fix spaces because it messes with multivarite stuff


# kelp.spp =  c("MACPYR", "NERLUE", "PTECAL")

# seletct depths and turn into data fram. tidyverse stuff messes with ordingations

# kelp <-    data.frame(kelp %>% filter(zone %in% kelp.depth))
# fish <-    data.frame(fish%>% filter(zone %in% fish.depth))
# inverts <- data.frame(inverts%>% filter(zone %in% invert.depth))


# need regular data frames or the column tites mess with the ordination software
...
```{r Figure-3, fig.width=6, fig.height=8}
...
# check the rownames
# rownames(yoy_ord$cap$cproj)
# relabel
rownames(yoy_ord$cap$cproj) <- c('copper','canary','blue','black/yellowtail','rockfish yoy')
...
Plot_Ordination( data.file = kelp_ord$cap.df , ord.file = kelp_ord$cap, min.score = 0.2,
                 ...
                 Xlim = c(-3,3), Ylim=c(-3,3), Xlim2 = xy2, Ylim2=xy2, scores.cex = 1.0, scores.font = 3,
...
Plot_Ordination( data.file = invert_ord$cap.df , ord.file = invert_ord$cap, min.score = 0.2, pval = p_inv, 
                 Yform = 'LD', Xform =  ' ~ site + year + col + pch',scores.cex = 1.0,
...
Plot_Ordination( data.file = fish_ord$cap.df , ord.file = fish_ord$cap, min.score = 0.2,
                 ...
                 Xlim = c(-3,3), Ylim=c(-3,3), Xlim2 = xy2, Ylim2=xy2,scores.cex = 1.0,
...
Plot_Ordination( data.file = yoy_ord$cap.df , ord.file = yoy_ord$cap, min.score = 0.2,
                 ...
                 Xlim = c(-5,5), Ylim=c(-5, 5), Xlim2 = xy2, Ylim2=xy2,scores.cex = 1.0,
...
``` {r Figure-4, fig.width=4.5, fig.height=3.5}
...
```{r ordinations-5m, include = FALSE}
## these analyses are run at the transect level
## later graphing may summarize them at different scales
...
```{r ordinations-plots-separate-spp-panes-5m, fig.width=6, fig.height=8}
...
``` {r explained-variance-5m, fig.width=4.5, fig.height=3.5}
