...
run.ord = FALSE
nperm = 999
transform = 'log' # choose transform here
if(run.ord == TRUE){
...
saveRDS(kelp_ord,file = paste0(Results_Loc,"Kelp-ordination-n",nperm,"-",transform,".rds"))
...
saveRDS(fish_ord,file = paste0(Results_Loc,"Fish-ordination-n",nperm,"-",transform,".rds"))
...
saveRDS(yoy_ord,file = paste0(Results_Loc,"YOY-ordination-n",nperm,"-",transform,".rds"))
...
saveRDS(invert_ord,file = paste0(Results_Loc,"Invert-ordination-n",nperm,"-",transform,".rds"))

}else{ # reload previously run files to save time
     kelp_ord    <- readRDS( paste0(Results_Loc,"Kelp-ordination-n",nperm,"-",transform,".rds"))
     fish_ord   <- readRDS( paste0(Results_Loc,"Fish-ordination-n",nperm,"-",transform,".rds"))
     yoy_ord    <- readRDS( paste0(Results_Loc,"YOY-ordination-n",nperm,"-",transform,".rds"))
     invert_ord <- readRDS( paste0(Results_Loc,"Invert-ordination-n",nperm,"-",transform,".rds"))
}
...
Ordinations use a `r transform` transform to the data to reduce the influence of abundant species.
...
df1 = df0[,c('id','site.x','year.x','zone.x','area.x', cap.fish.spp,algae.spp)]
rownames(df1) <- df1$id
df = na.omit(df1)
if(set.depth.5m == TRUE){df = df[df$zone.x == 5,]}
cap.fish = df[,c(cap.fish.spp)]
cap.fish = log(cap.fish+1)
transform = 'log'

dim(cap.fish)

cap1 = capscale( cap.fish ~ MACPYR + NERLUE + PTECAL , data = df, dist="bray" )
anova(cap1)
...
x = scores(cap1)
df = cbind(df,x$sites)
df$col = site.col$col[match(df$site.x,site.col$s)]
...
saveRDS(list(cap1,df), file = paste0(Results_Loc,'Fish-Kelp-',transform,".rds"))
...
This includes the selected adult/large fish and sebastes yoy in the same redundancy analysis.

```{r fish-all-kelp-capscale}
# distance-based redundancy analysis

df0 = readRDS(paste0(Data_Loc,'Data_Fish_x_Kelp.rds'))

cap.fish.spp = c("EMBI","GOBI","OPEL", "HEXA", "SECA", "SCMA" ,"SENE", "SEME", "SEFL", "SECAyoy", "SEPIyoy", "YTBLyoy","RYOY")
