...
SP.yoy <- c("SEBYTyoy", "YTB",  'black',
            ...
            "SEMYyoy", "Blue RF" ,                fish.col[2],
            #"SENEyoy", "China",                fish.col[10],
            "SEPIyoy", "Canary RF",               fish.col[6],
            "RYOY", "Unidentified RF",            'darkgrey') # ,
...
`%!in%` <- Negate(`%in%`)
algaex$spp = algaex$species
algaex$spp[ algaex$species %!in% c('MACPYR', 'NERLUE', 'PTECAL')] <- "Other"

algae3 <- algaex %>% 
  group_by(year,site,spp,zone) %>%
  summarise(mean.density=mean(density, na.rm = TRUE), 
            SD = sd(density), 
            N =length(year),
            SE= SD/sqrt(N) ) 
...
```{r big3, fig.width=6, fig.height=4.5}
 # big three species
 big3spp = c('MACPYR', 'NERLUE', 'PTECAL', 'Other')
 big3 <- algae3 %>% filter(spp %in% big3spp)
 big3$site <- factor(big3$site,levels=sites)
 big3$spp <- factor(big3$spp, levels=big3spp)
 
 kelp.labs = c('Macrocystis','Nereocystis','Pterygophora','Other')
 ...
 kelp1 = 
   ...
   ggplot(big3) +
    ...
    facet_grid(spp ~ zone,scales="free_y", labeller = labeller(zone=depth.labs , spp=kelp.labs)) +
    scale_x_continuous(breaks=seq(2015,2021)) +
    xlab("Year")+
    scale_y_continuous(expression("Stipes per m"^2)) +
    # scale_color_viridis_d(option="plasma",end=0.75)+
    scale_color_manual(values = site.col$col)+
    theme_bw()+ theme_nt + 
...
kelp_transect$spp = kelp_transect$species
`%!in%` <- Negate(`%in%`)
kelp_transect$spp [ kelp_transect$species %!in% c("MACPYR",'NERLUE','PTECAL')] <- 'Other'

x <-  kelp_transect %>% filter(., spp %in% c("MACPYR",'NERLUE','PTECAL', 'Other')) %>% group_by(site,year,spp) %>%
...
dfx$spp[dfx$spp=='MACPYR'] <- 'Macrocystis'  
dfx$spp[dfx$spp=='NERLUE'] <- 'Nereocystis'  
dfx$spp[dfx$spp=='PTECAL'] <- 'Pterygophora'

dfx$spp <- factor(dfx$spp, levels = c('Macrocystis','Nereocystis','Pterygophora','Other'))

kelp_coast <- ggplot( dfx , aes(x=year, y=grand_mean, color=spp)) +
                geom_point() +
                geom_line() +
                geom_errorbar(aes(x=year,
                                  ymin=grand_mean-grand_SE,
                                  ymax=grand_mean+grand_SE, color=spp),width=0) +
                scale_color_manual(values = site.col$col)+
                scale_x_continuous(breaks=seq(2015,2021)) +
                xlab("")+
                scale_y_continuous(expression("Kelp stipes per m"^2)) +
                theme_bw()+ theme_nt +
...
tat_urchin2021 = urchin_zone[urchin_zone$year==2021 & urchin_zone$site =='Tatoosh Island',]
...
urchin_plot_all <- ggplot(data=urchins_all,aes(x=year, y= mean.density, col = site)) +
                    ...
                    scale_y_continuous(expression("Urchins per m"^2)) +
                    theme_bw()+ theme_nt
...
tat_purpleurchins2021 = urchins_all[urchins_all$year==2021 & 
                                    urchins_all$site =='Tatoosh Island' &
                                    urchins_all$taxa == 'Purple urchin',]
```
...
urchin.plot <- ggplot(urchin_year, aes(x = year, y=grand_mean, color=taxa)) +
                ...
                scale_x_continuous("", breaks=seq(2015,2021)) +
                scale_y_continuous(expression("Urchins per m"^2), limits=c(0,3)) +
                theme_bw()+ theme_nt 
                # theme(legend.position = c(0.2, 0.8),
...
x2021 = urchin_year$grand_mean[urchin_year$year == 2021 & urchin_year$taxa == "Purple urchin"]
...
Purple urchin density in 2015 = `r x2019` urchins per m^2
...
samples_table
write.csv(samples_table, paste0(Fig_Loc,'Table-sampling-effort.csv'), row.names = FALSE)
...
saveRDS(fish_kelp_count_wide , file=paste0(Data_Loc,"Fish-kelp-counts-wide.rds"))
...
fish_total <- fish_transect %>% group_by(species, size_class) %>%
...
fish_total <- fish_total[,c('spp', 'com_name', 'size_class', 'group', 'Total')] %>%
              rename(Species = spp) %>%
...
write.csv(fish_adults, paste0(Fig_Loc,"Table-Fish-Adults.csv"))
write.csv(fish_juveniles, paste0(Fig_Loc,"Table-Fish-Juveniles.csv"))
...
kelp_total <- kelp_total[,c('Species','Density','SD')]
...
```{r Figure-2, fig.width=6.65, fig.height=6.65}

# library(ggpubr)
canopy.kelp <- canopy.kelp + theme(legend.position = c(0.8 , 0.6) )
kelp_coast  <- kelp_coast +  
  scale_y_continuous(expression("Kelp stipes per m"^2), breaks=c(0,1,2), limits=c(0,2.2)) +
  theme(legend.position = c(0.80 , 0.85) )

urchin.plot <- urchin.plot + theme(legend.position = c(0.30 , 0.85) )
stars_year  <- stars_year  + 
  scale_y_continuous(expression("Sea stars per m"^2), breaks=c(0,0.1,0.2), limits=c(0,0.2)) +
...
df <- data.frame(read_excel( paste0(Data_Loc,
      'Mean Daily OISST (C) for OCNMS sites 2003 to 2021.xlsx'), sheet = 1))
rownames(df)
# get just our sites
...
canopy.kelp.long <- ggplot( canopy %>% filter(., site == 'Coastwide') %>% filter(.,year %in% 2003:2021), 
                       aes(x=year, y = canopy_ha, color = species)) +
                geom_point() +
                geom_line() +
                xlab("") +
                scale_x_continuous(breaks = seq(2005,2020,5), 
                                   minor_breaks = 2003:2022, 
                                   limits = c(2003,2021) ) +
                scale_y_continuous("Canopy area (ha)") +
                scale_color_manual(values = site.col$col[c(5,1,2,3)],
                                   labels = c('Total', 
...
```{r Fig-1-sst-kelp, fig.width=3.18 , fig.height=5, eval=FALSE}

plot6 = readRDS(file = "Fig-1-Mean-sst-warmest-month-by-site.rds")

sst = plot6 + theme_bw()+theme_nt + theme(legend.position = c(0.4,0.9))
