---
title: "Invert Mesopredator Community ID"
...
```{r}
dat_zone %>%  
 filter(lump %in% c( "kelp_crab"), !year ==2015 ) %>% 
...
```{r invert diversity }
total_proportion <-  dat_zone %>%
   filter(!year ==2015, !dens_weighted == 0#, !site=="Tatoosh Island"
          )  %>%
  group_by(year,site,zone) %>%
    dplyr::summarise(
            total_sum=sum(dens_weighted))
  
dat_zone %>% #group_by(year,site,zone,lump) %>%
   filter(!year ==2015, !dens_weighted == 0#, !site=="Tatoosh Island"
          )  %>%
  left_join(total_proportion) %>%
  group_by(year,site,zone,lump) %>%
    dplyr::summarise(
            PROPORTION=dens_weighted/total_sum,
            PROPORTION_2=sum(PROPORTION)^2,
             .groups = "keep") %>%
      group_by(year,site,zone) %>%
    dplyr::summarise(    
            DIVERSITY= sum(PROPORTION_2),
             SIMPSONS_DIVERSITY= 1 - DIVERSITY, # simpsons diversity
            .groups = "keep") %>%
    select(-DIVERSITY) %>%
  group_by(site) %>%
    dplyr::summarise(
      MEAN=mean(SIMPSONS_DIVERSITY),
      SD= sd(SIMPSONS_DIVERSITY),
      N=length(SIMPSONS_DIVERSITY),
      SE=SD/sqrt(N),
      .groups = "keep"
      ) %>% 
  mutate(site = factor(site, levels =c("Destruction Island","Cape Johnson","Cape Alava","Tatoosh Island" , "Neah Bay" ))) %>%
  ggplot(aes(x=site, y =MEAN))+
    ...
    theme_classic() +
    ylab("Simpsons Diversity Index - inverts")
```


```{r}
color6= pnw_palette("Cascades", "continuous", n=100)

dat_zone %>%  
 filter(!year ==2015,
        !lump%in% c("Cal_cuc", "large_anemone","large_nudibranch","med_nudibranch","no_org","orange_cucumber","sea_cucumber", "small_anemone", "sponge", "tunicate")) %>% 
   group_by(year,site,lump) %>%
 dplyr::summarise(dens_weighted = mean(dens_weighted)) %>% #take mean across zones 
  ungroup() %>%
  mutate(site = factor(site, levels =c("Destruction Island","Cape Johnson","Cape Alava","Tatoosh Island" , "Neah Bay" ))) %>%
ggplot(aes(fill=lump, y=dens_weighted, x=year)) + 
    geom_bar(position="stack", stat="identity") +
 # scale_fill_manual(values = color6) +
  ggtitle("2016-2019 Invert Community Diversity") +
