...
macro = filter(df_diff, kelp_spp == "Macrocystis")
...
df_diff = nereo %>% group_by(kelp_spp, site) %>%
  mutate( Kelp = Kelp - lag(Kelp, default = Kelp[1]),
          Urchins = Urchins - lag(Urchins, default = Urchins[1])) %>%  filter(year>2015)
...
df_diff = ptery %>% group_by(kelp_spp, site) %>%
  mutate( Kelp = Kelp - lag(Kelp, default = Kelp[1]),
          Urchins = Urchins - lag(Urchins, default = Urchins[1])) %>%  filter(year>2015)
...
df_diff = other %>% group_by(kelp_spp, site) %>%
  mutate( Kelp = Kelp - lag(Kelp, default = Kelp[1]),
          Urchins = Urchins - lag(Urchins, default = Urchins[1])) %>%  filter(year>2015)
...
plot1 <- ggplot( df_diff, aes(x = Urchins, y = Kelp, color=group)) +
  ...
  # annotate(geom="text", x=stats.x, y=1.0, label = expression(italic("Pterygophora")), hjust = 0, color=site.col$col[3], size = 3) +
  # annotate(geom="text", x=stats.x, y=0.7, label = rstatement3, hjust = 0, color=site.col$col[3], size = 3) +
  # annotate(geom="text", x=stats.x, y=0.4, label = pstatement3, hjust = 0, color=site.col$col[3], size = 3) +
          theme_bw() + theme_nt 
...
neah2 <- neah1 +   theme(legend.position = "none") + 
                         annotate(geom="text",x=-0.07,y=3.4, label="f)", size=SIZE)

nereo.tat2 <- nereo.tat +theme(legend.position = c(0.75,0.2))+ 
                         annotate(geom="text",x=-6.0,y=10, label="g)", size=SIZE)

ptery.tat2 <- ptery.tat +theme(legend.position = 'none')+ 
                         annotate(geom="text",x=-6.0,y=10, label="h)", size=SIZE)

other.tat2 <- other.tat +theme(legend.position = 'none')+ 
                         annotate(geom="text",x=-6.0,y=10, label="i)", size=SIZE)
