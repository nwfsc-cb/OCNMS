...
df = df_transect %>% group_by(site, year, zone, area, transect, kelp_spp) %>%
                      summarise(urchins = sum(urchin_density), kelp = mean(kelp_density) ) %>% 
                      group_by(site,year,kelp_spp) %>%
                      summarise(Urchins = sum(urchins), Kelp = mean(kelp) )
...
macro1 <- ggplot(macro, aes( x=Urchins , y=Kelp, color=site)) +
         ...
         xlim(0,NA)+
         ylim(0,NA)+
         # geom_smooth(data = filter(macro,site =='Tatoosh Island' ), method='lm', se=FALSE) +
         xlab( expression(paste('Urchins per 60 ', m^2) ) )+
         ylab( expression(paste(italic('Macro'), ' stipes per 60 ', m^2)) )+
         theme_bw() + theme_nt 
...
ptery1 <- ggplot(ptery, aes( x=Urchins , y=Kelp, color=site)) +
         ...
         annotate(geom="text", x=40, y=1.0, label = "Tatoosh", 
                  hjust = 0, color=site.col$col[2], size = 3) +
         annotate(geom="text", x=40, y=0.7, label = rstatement, hjust = 0, color=site.col$col[2], size = 3) +
         annotate(geom="text", x=40, y=0.4, label = pstatement, hjust = 0, color=site.col$col[2], size = 3) +
         xlab( expression(paste('Urchins per 60 ', m^2) ) )+
...
plot_nereo <- ggplot(df_nereo , aes(x = Urchins, y = Kelp )) +
  ...
  xlim(0,NA) + 
  ylim(0,NA)+
  annotate(geom="text", x=1.5, y=1.0, label= rstatement, hjust = 0, color="black", size = 3) +
...
nereo.tat = ggplot(dfx , aes(x = urchins, y = kelp)) + 
        ...
        geom_text(data=dfx , aes(x=urchins, y=kelp, label=pch), size = 3)+ 
        xlab( expression(paste('Urchins per 60 ', m^2) ) )+
        ylab( expression(paste(italic('Nereo'), ' stipes per 60 ', m^2)) )+
...
plot1 <- ggplot( df%>%filter(df$group %in% c('Macrocystis','Nereocystis','Pterygophora')) ,
          ...
          annotate(geom="text", x=stats.x, y=1.7, 
                   label = rstatement2, hjust = 0, color=site.col$col[3], size = 3) +
          annotate(geom="text", x=stats.x, y=1.4, 
...
```{r Figure-5, fig.height=6, fig.width=6}

plot1  <- plot1 +  theme(legend.position = c(0.8,0.2), 
...
ggarrange(plot1, macro1, nereo1, ptery1, nereo.tat, ptery.tat,
           labels = c('a)','b)','c)','d)','e)','f)'), 
           font.label = list(face='plain', size = 10),
           hjust = c(-5.5, -23.5,-5.5,-23.5, -5.5,-8.5),
           vjust = 2,
           align = 'v',
           nrow = 3, ncol=2)
