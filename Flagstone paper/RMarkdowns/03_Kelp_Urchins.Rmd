...
```{r Supplement-kelp-vs-urchins-transet}
library(cowplot)
library(viridis)
...
plot_canopy <- ggplot(df_canopy , aes( x = Urchins, y = Kelp )) +
  ...
  # scale_color_manual(values=site.col$col)+
  xlab('Urchin density') +
  ylab("Canopy kelp stipe density") +
  xlim(0,NA) + 
  ylim(0,NA)+
...
plot_nereo <- ggplot(df_nereo , aes(x = Urchins, y = Kelp )) +
  ...
  geom_text(aes(x=Urchins,y=Kelp, label=year), size = 3)+
  # scale_color_manual(values=site.col$col)+
  xlab('Urchin density') + ylab("Nereocystis stipe density") +
  xlim(0,NA) + 
  ylim(0,NA)+
...
plot_ptero <- ggplot(df_ptero , aes(x = Urchins, y = Kelp )) +
  ...
  geom_text(aes(x=Urchins,y=Kelp, label=year), size = 3)+
  # scale_color_manual(values=site.col$col)+
  ylab("Pterygophra stipe density") +
  xlab('Urchin density') +
  xlim(0,NA) + 
  ylim(0,NA)+
...
p.tat = ggplot(dfx , aes(x = urchins, y = kelp)) + 
        ...
        geom_smooth( method = "nls",
                      ...
                      method.args = list(start=c(a=1,k=0) ),
                      se=FALSE) + 
        xlab('Total urchin density')+
        ylab('Kelp stipe density') + 
        theme_bw() + theme_nt
...
nereo.tat = ggplot(dfx , aes(x = urchins, y = kelp)) + 
        ...
        geom_text(data=dfx , aes(x=urchins, y=kelp, label=pch), size = 4)+ 
        xlab('Urchin density')+
        ylab('Nereocystis stipe density') +
...
ptery.tat = ggplot(dfx , aes(x = urchins, y = kelp)) + 
        ...
        geom_text(data=dfx , aes(x=urchins, y=kelp, label=pch), size = 4)+ 
        xlab('Urchin density')+
        ylab('Pterygophora stipe density') +
        annotate(geom="text", x=15, y=30, label = rstatement, hjust = 0, color="black", size = 3) +
        annotate(geom="text", x=15, y=27, label= pstatement, hjust = 0, color="black", size = 3) +
        theme_bw() + theme_nt
...
```{r urchins-kelp-tatoosh, fig.height=4, fig.width=5}


library(ggpubr)

ggarrange( nereo.tat, ptery.tat, plot_tatoosh_year)
...
ggarrange( plot_tot, plot_canopy, plot_nereo, plot_macro, plot_ptero,
           labels = c('a)','b)','c)','d)', 'e)'), 
