...
plot_transect <- ggplot( df_tat , aes( y=kelp_density , x=urchin_density )) + 
...
nereo.cor = cor( dfn$kelp_mean , dfn$urchin_mean )
dfp = df_tat %>% filter(kelp_spp == 'Pterygophora')
ptero.cor = cor( dfp$kelp_mean , dfp$urchin_mean )

plot_site <- ggplot( df_tat , aes( x=kelp_mean , y=urchin_mean )) + 
  #geom_point( size = 4, pch = df_tat$pch ) +
  geom_text(data=df_tat , aes(y=kelp_mean,x=urchin_mean, label=pch2), size = 3)+
...
Nereocystis vs Urchins r = `r nereo.cor`
Pterygophora vs Urchins r = `r ptero.cor`
...
```{r total-kelp-urchins-site-year, fig.width=3.5, fig.height=2.5}
...
plot_site_tot <- ggplot(df_tot , aes(y = kelp , x = urchins, color = site)) +
...
plot_tot <- ggplot(df_tot , aes(x = Kelp , y = Urchins)) +
  # geom_point(size = 5 , show.legend = FALSE) +
  geom_text(aes(y=Kelp,x=Urchins, label=year), size = 3)+
  # scale_color_manual(values=site.col$col)+
  ylab("Total kelp density") +
  xlab('Urchin density') +
  xlim(0,NA) + 
  ylim(0,NA)+
  theme_bw() + theme_nt + 
...
plot_canopy <- ggplot(df_canopy , aes(x = Kelp , y = Urchins)) +
  # geom_point(size = 5 , show.legend = FALSE) +
  geom_text(aes(y=Kelp,x=Urchins, label=year), size = 3)+
  # scale_color_manual(values=site.col$col)+
  ylab("Canopy kelp density") +
  xlab('Urchin density') +
  xlim(0,NA) + 
  ylim(0,NA)+
  theme_bw() + theme_nt + 
...
plot_ptero <- ggplot(df_ptero , aes(x = Kelp , y = Urchins)) +
  # geom_point(size = 5 , show.legend = FALSE) +
  geom_text(aes(y=Kelp,x=Urchins, label=year), size = 3)+
  # scale_color_manual(values=site.col$col)+
  ylab("Pterygophra density") +
  xlab('Urchin density') +
  xlim(0,NA) + 
  ylim(0,NA)+
  theme_bw() + theme_nt + 
...
p.tat = ggplot(dfx , aes(y = kelp, x = urchins)) + 
        # geom_point() +
        geom_text(data=dfx , aes(x=kelp,y=urchins, label=pch), size = 4)+ 
...
nereo.tat = ggplot(dfx , aes(y = kelp, x = urchins)) + 
        # geom_point() +
        geom_text(data=dfx , aes(x=kelp,y=urchins, label=pch), size = 4)+ 
        geom_smooth( method = "nls",
                     formula=y ~ a * exp(k * x),
...
ggarrange( plot_transect, plot_site)
...
```{r Figure-6, fig.height=6, fig.width=3.5}


library(ggpubr)

ggarrange( plot_tot, plot_canopy, plot_ptero,
           labels = c('a)','b)','c)'), 
