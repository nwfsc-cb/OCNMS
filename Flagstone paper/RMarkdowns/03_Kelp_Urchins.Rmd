...
dfx$p1 = as.numeric(p1)

# combine to plot
df_tat <- df_tat %>% left_join(., dfx)


sg1

plot_transect <- ggplot( df_tat , aes( x=kelp_density , y=urchin_density )) + 
  geom_point( size = 3, pch = df_tat$pch , show.legend = FALSE ) +
  facet_wrap(facets = df_tat$kelp_spp, ncol=1)+
  # geom_line(  data=df_tat %>% filter(kelp_spp == "Nereocystis"), aes(x = kelp_density , y = p1))+
  xlim(0,4)+
  ylim(0,NA)+
  xlab("Kelp density") +
  ylab('Urchin density') +
...
plot_site_tot <- ggplot(df_tot , aes(x = kelp , y = urchins, color = site)) +
...
plot_tot <- ggplot(df_tot , aes(x = Kelp , y = Urchins)) +
  ...
  geom_text(data=df_tot , aes(x=Kelp,y=Urchins, label=year), size = 3)+
  # scale_color_manual(values=site.col$col)+
  xlab("Kelp density") +
  ylab('Urchin density') +
  xlim(0,12) + 
...
KU_year = cor(df_tot$Kelp , df_tot$Urchins)

plot_tot
```


Kelps and Urchins sites averaged across years, all kelps (Macro, Ptero, Nereo) r = `r KU_year`


```{r urchins-kelp-all, fig.height=2.5, fig.width=5}


library(ggpubr)

plot_site_tot <- plot_site_tot + theme(legend.position = c(0.3 , 0.75)) 
plot_tot <- plot_tot + xlim(0,11.5) + ylim(0,NA)


ggarrange( plot_tot, plot_site_tot,
           labels = c('a)','b)'), 
