...
# Pad with zero observations. i.e., complete the data so that each segment has all 4 substrate categories
  ...
  # 2) calculate substrate diversity for each transect
  ...
  # 3) calculate summary stats for site-year-zone
  ...
  # 4) calculate substrate diversity for each site-year-zone
  sub.summary.year.zone.div <- sub.summary.year.transect.div %>% 
    group_by(YEAR,SITE,ZONE) %>%
    dplyr::summarise(
      SUBSTRATE_DIVERSITY=mean(SUBSTRATE_DIVERSITY),
      SD_SUBSTRATE_DIVERSITY= sd(SUBSTRATE_DIVERSITY),
      N=length(SUBSTRATE_DIVERSITY),
      SE=SD_SUBSTRATE_DIVERSITY/sqrt(N),
      .groups = "keep"
      )
