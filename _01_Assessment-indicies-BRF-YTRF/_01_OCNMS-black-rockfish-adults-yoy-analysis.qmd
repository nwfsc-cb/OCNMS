---
title: "Black Rockfish Analysis OCNMS (Full: Adults and YOY)"
...
dat.bk <- dat.fish %>% filter(species %in% SPECIES) %>%
              bind_rows(.,
...
# source("~/Github/OCNMS/_01_Assessment-indicies-BRF-YTRF/Process Length Structure-BLYT.R")

load( paste0(work_dir, SPP, "_lengths_2015-", survey_year,".Rdata") )
# relevant object here is "length.dat"

```

# Young of year analysis for `r SPP`, *`r sciname`*

## Data description

These are derived data products relevant to estimating `r spp` abundance and size stemming from NWFSC dive surveys in the Olympic Coast National Marine Sanctuary (OCNMS) conducted between 2015 and 2022. Description of survey methods and aims are detailed in [@tolimieri2023changes] We provide a brief summary of methods relevant to fish surveys here.
...
As a result of our 2-m visibility requirements, there were transects that were omitted from the analyses. The following tables ([@tbl-2m-samples] & \[@tbl-2m-samples\]) show is how the number fish transects with visibility \>2 m were distributed across sites and years. 2015 includes only surveys conducted at 5-m depth. Other years have data approximately evenly split between 5-m and 10-m depths.
...
### Large (\>10 cm total length) `r SPP` trends

To calculate the average density of `r spp` in each year, we first calculate the mean density and standard error per site in each year. This approach means we are treating each transect as a i.i.d. sample of `r spp` density within each site and thus we ignore differences in abundance by depth zone.
...
\end{align}
```
and then the predicted density for a single realization in a given year is the mean among sites observed. We repeat the simulation 100,000 times to provide an estimated mean density and uncertainty for a given year ([Fig. @fig-spp-ts]).
...
#check to see that this only contains large black rockfish 
# (should be species "SEME")
#dat.bk.summary %>% distinct(species,year,size_class)
...
### Information on size data for `r spp` 2015-`r survey_year`
...
p.spp.size.bin1 <- ggplot(dfx) +
  ...
  theme( axis.text.x = element_text(size=7),
         strip.text.y.right = element_text(8) )
#p.spp.size.bin1
...
Plots of size distribution grouped into 5 cm bins ([Figs. @fig-spp-size-bin1] & [-@fig-size-bin2]).
...
#| warning: false
#| fig-width: 6
#| fig-cap: "YOY size distribution by 5-cm size bins."
...
#| warning: false
#| fig-height: 10
#| fig-cap: "YOY size distribution by 5-cm size bins plotted by site."
...
While nominally sizes are recorded in 1cm increments, in practice sizes in 5cm increments are recorded ([Fig. @fig-size-dist2]).
...
#| warning: false
#| fig-height: 10
#| fig-cap: "Plots of size distribution, but without 5cm bins."
...
save(bk.rockfish.output, file= paste0(work_dir,SPP,"_OCNMS_indices.Rdata"))
```
