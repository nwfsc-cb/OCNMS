...
author: Nick Tolimieri with KF and GW
date: sys.date()
output: 
  html_document:
    self_contained: true
...
**Before starting set the working directory by:**
...
# Paths for files
# set wd to source loc first
home_dir = getwd()
fig_dir = paste0(home_dir,"/figures/")
ocnms_dir = stringr::str_remove(home_dir,'_00_Annual-Update')
data_dir = paste0(ocnms_dir,"Data/",data_year,"/")
...
Errors below are likely due to a new species being added. You may need to update the codes file for fish, kelp, or swath. 
...
```{r data-import}
# fish
GW = stringr::str_detect(home_dir, "greg.williams")
OS = stringr::str_detect(home_dir, "ole.shelton")

# if(OS==TRUE){
#   fish0 = readRDS(
#   paste0("/Users/ole.shelton/Documents/GitHub/OCNMS/Data/2025/Fish_2015-",data_year,'.rds' ))
#   swath0 = readRDS(
#   paste0("/Users/ole.shelton/Documents/GitHub/OCNMS/Data/2025/Swath_2015-",data_year,'.rds' ))}
# 
# 
# if(GW==TRUE){
#   fish0 = readRDS(
#   paste0("/Users/greg.williams/Documents/GitHub/OCNMS/Data/2025/Fish_2015-",data_year,'.rds' ))
#   swath0 = readRDS(
#   paste0("/Users/greg.williams/Documents/GitHub/OCNMS/Data/2025/Swath_2015-",data_year,'.rds' ))
#   
# }else{
...
NT = stringr::str_detect(home_dir,'nick.tolimieri')
if(NT == TRUE){write.csv(esr_yoy, "~/GitHub/NMS-condition-reports/data/ocnms-dive-fish-yoy.csv", row.names = F)}
...
# ocnms condition report
if(NT == TRUE){write.csv(esr_fish, "~/GitHub/NMS-condition-reports/data/ocnms-dive-fish-adults.csv", row.names = F)}
...
```{r nerocystis-tatoosh , fig.width=5, fig.height=3.5}
 # Basic time-series by site and zone TATOOSH FOCUS
nereo_tat =  ggplot(canopy %>% filter(site=="Tatoosh Island",species=="NERLUE")) +
   ...
   scale_y_continuous(expression("Nero stipes m"^-2),limits=c(0,NA)) +
...
```{r nerocystis-tatoosh-mean , fig.width=5, fig.height=3.5}
 # Basic time-series by site and zone TATOOSH FOCUS

algaex <- algae1 %>% 
  group_by(year, site, species, fun_gr) %>%
  summarise(mean.density=mean(density, na.rm = TRUE), 
            SD = sd(density), 
            N =length(year),
            SE= SD/sqrt(N) ) 

nereo_tat =  ggplot(algaex %>% filter(site=="Tatoosh Island",species=="NERLUE")) +
   geom_point(aes(x=year,y=mean.density),alpha=0.5) +
   geom_line(aes(x=year,y=mean.density),alpha=0.5) +
   geom_errorbar(aes(x=year,ymin=mean.density-SE,ymax=mean.density+SE),width=0) +
   #facet_grid(rows="species",scales="free_y") +
   scale_x_continuous(breaks=seq(2015,last.year,2)) +
   scale_y_continuous(expression("Nero stipes m"^-2),limits=c(0,NA)) +
   scale_color_viridis_d("Depth (m)",option="plasma",end=0.75)+
...
## Stipe counts for *Nerocystis*, *Macrocystis*, and *Pterogophora*

### Site x species x depth 
...
df_out = dfx %>% rename(timeseries=species, index=grand_mean, SE=grand_SE) %>%
  mutate(metric="Kelp stipes m2") %>%
  dplyr::select(year, index, SE, timeseries, metric)

# write out to OCNMS condition report
if(NT == TRUE){write.csv(df_out, "~/GitHub/NMS-condition-reports/data/ocnms-dive-kelp.csv", row.names = F)}
...
Same as above but subset out for just 5 m. Mean ignores 10-m transects. 
...
```{r kelp-coastwide-total, fig.height=3.5, fig.width=5}

# mean coastwide across depths

dfx <-  kelp_transect %>% 
      filter(., species %in% c("MACPYR",'NERLUE','PTECAL')) %>% 
      group_by(site,year,species) %>%
      summarise(Mean=mean(density, na.rm = TRUE),
                SD=sd(density),
                N=length(density),
...
kelp_coast <- ggplot( dfx , aes(x=year, y=grand_mean)) +
                geom_point() +
                geom_line() +
                geom_errorbar(data=dfx, aes(x=year,
                                  ymin=grand_mean-grand_SE,
                                  ymax=grand_mean+grand_SE),width=0) +
                # scale_color_manual(values = site.col$col)+
                scale_x_continuous(breaks=seq(2015,last.year,2)) +
                xlab("")+
                scale_y_continuous(expression("Kelp stipes m"^-2)) +
...
dfx <-  kelp_transect %>% filter(species %notin%
                                  c("MACPYR",'NERLUE','PTECAL',"NO_ALG")) %>% group_by(site,year,species) %>%
      summarise(Mean=mean(density, na.rm = TRUE),
                SD=sd(density),
                N=length(density),
                SE=SD/sqrt(N),
...
kelp_coast
dfx$timeseries = kelp_codes$spp[match(dfx$species,kelp_codes$species)]


df_out = dfx %>% rename(index=grand_mean, SE=grand_SE) %>%
  mutate(metric="Kelp stipes m2") %>%
  dplyr::select(year, index, SE, timeseries, metric)

# write out to OCNMS condition report
if(NT == TRUE){write.csv(df_out, "~/GitHub/NMS-condition-reports/data/ocnms-dive-understory-kelp.csv", row.names = F)}
```
...
## Pycnopodia - total on all transects by year - table
...
ggplot(pycnos, aes(year, no)) +
  ylab("Total observed across five sites")+
  scale_x_continuous(breaks=2015:data_year) +
  geom_line() + geom_point()+theme_bw()


p_out = pycnos %>% rename(index = no) %>%
  mutate(timeseries="Pycnopodia", metric = "Total observed per 5 sites")

if(NT == TRUE){write.csv(p_out, '~/GitHub/NMS-condition-reports/data/ocnms-dive-total-pycno.csv', row.names = F)}

```

```{r mean-pycnopodia}

df = invert_transect %>% group_by(year, species) %>%
  summarise(mn = mean(density),
            vr = var(density),
            n = n()) %>%
  mutate(se = vr/sqrt(n))


ggplot(df %>% filter(species=="PYCHEL"), aes(year, mn)) +
  geom_line() + geom_point() + 
  geom_errorbar( aes(ymin = mn-se, ymax=mn+se), width=0) +
  xlab("") + ylab("Pycnopodia per m2") + 
  scale_x_continuous(breaks=2015:data_year) +
  theme_bw()

df_out = df %>% filter(species=="PYCHEL") %>% 
  rename(index = mn, SE = se, timeseries = species) %>% mutate( metric = "No per 120 m^2")

if(NT == TRUE){write.csv(df_out, "~/GitHub/NMS-condition-reports/data/ocnms-dive-pynopodia.csv", row.names = F)}

```
```{r mean-pycnopodia-site}

df = invert_transect %>% group_by(year, species, site) %>%
  summarise(mn = mean(density),
            vr = var(density),
            n = n()) %>%
  mutate(se = vr/sqrt(n))
...
ggplot(df %>% filter(species=="PYCHEL"), aes(year, mn, color=site)) +
  geom_line() + geom_point() + 
  geom_errorbar( aes(ymin = mn-se, ymax=mn+se), width=0) +
  xlab("") + ylab("Pycnopodia per m2") + 
  scale_x_continuous(breaks=2015:data_year) +
 ...
 df_out = df %>% filter(species=="PYCHEL") %>% 
   rename(index = mn, SE = se, timeseries = species) %>% mutate( metric = "No per m2")

 if(NT == TRUE){write.csv(df_out, "~/GitHub/NMS-condition-reports/data/ocnms-dive-pynopodia-site.csv", row.names = F)}

```
```{r mean-pycnopodia-tatoosh}
...
ggplot(df %>% filter(species=="PYCHEL" & site=='Tatoosh Island'), aes(year, mn)) +
  geom_line() + geom_point() + 
  geom_errorbar( aes(ymin = mn-se, ymax=mn+se), width=0) +
  xlab("") + ylab("Pycnopodia per m2") + 
  scale_x_continuous(breaks=2015:data_year) +
...
# write.csv("~/GitHub/NMS-condition-reports/data/ocnms-dive-pynopodia.csv", row.names = F)
...
### Total urchin density - site x year
...
```{r total-urchins-mean, fig.width=6, fig.height=3.5}
  # Zoom in on urchins
urchin1 <- invert_transect %>% filter(grp2=="URCHIN") %>%
    group_by(year,site,transect,observer, area,grp2) %>%
    summarise(tot.density = sum(density, na.rm = TRUE)) %>% 
    group_by(year,site,area,grp2) %>%
    summarise(mean.density=mean(tot.density, na.rm = TRUE), 
              SD = sd(tot.density), 
              N =length(year),
              SE= SD/sqrt(N) ) 
 
urchin_mean <- invert_transect %>% filter(grp2=="URCHIN") %>%
    group_by(year,site,transect,observer,area,grp2) %>%
    summarise(tot.density = sum(density, na.rm = TRUE)) %>% 
    group_by(year,site,grp2) %>%
    summarise(mean.density=mean(tot.density, na.rm = TRUE), 
              SD = sd(tot.density), 
              N =length(year),
              SE= SD/sqrt(N) ) 
...
ggplot() +
     # geom_point(data= urchin1,aes(x=year,y=mean.density,color=site)) +
      geom_point(data=urchin_mean,aes(x=year,y=mean.density,color=site)) +
      geom_line(data= urchin_mean,
                 aes(x=year,y=mean.density,color=site)) +
      geom_errorbar(data=urchin_mean,
...
### Coastwide total urchins

Mean urchin density across all sites. Species summed because mostly purple urchins.

```{r total-urchins-coast, fig.width=6, fig.height=3.5}
  # Zoom in on urchins

urchin_coast <- invert_transect %>% filter(grp2=="URCHIN") %>%
    group_by(year,site,transect,observer,area,grp2) %>%
    summarise(tot.density = sum(density, na.rm = TRUE)) %>% 
    group_by(year, grp2) %>%
    summarise(mean.density=mean(tot.density, na.rm = TRUE), 
...
ggplot() +
     # geom_point(data= urchin1,aes(x=year,y=mean.density,color=site)) +
      geom_point(data=urchin_coast,aes(x=year,y=mean.density)) +
      geom_line(data= urchin_coast,
                 aes(x=year,y=mean.density)) +
      geom_errorbar(data=urchin_coast,
                    aes(x=year,
...
### Coastwode Urchins vs kelp

```{r urchins-vs-kelp-coastwide, fig.width=5, fig.height=3}

df_urchin_coast = urchin_coast %>%
  dplyr::select(year, mean.density) %>%
  rename(mean_urchins = mean.density)
df_kelp_coast = df_kelp_coast %>%
  dplyr::select(year, grand_mean) %>%
  rename(mean_kelp = grand_mean)


df1 = left_join(df_urchin_coast, df_kelp_coast)


ggplot(df1, aes(mean_urchins, mean_kelp)) + 
  geom_point() +
  xlab("Urchin density") + ylab("Kelp stipe density") + 
  theme_nt + theme_bw() + 
  theme(axis.title = element_text(size=14),
        axis.text = element_text(size=14))


r = round(cor(df1$mean_urchins, df1$mean_kelp),2)

# ggsave("Urchins-vs-kelp-coast.png", width=5, heigh=3)

```

### Coastwode Urchins vs kelp Differenced

```{r urchins-vs-kelp-coastwide-diff, fig.width=5, fig.height=3}


dif_kelp = diff(df1$mean_kelp)
dif_urch = diff(df1$mean_urchins)
df_dff = cbind(dif_kelp, dif_urch)  

ggplot(df_dff, aes(dif_urch, dif_kelp)) + 
  geom_point() +
  xlab("Change in urchin density") + ylab("Change in kelp stipe density") + 
  theme_nt + theme_bw() + 
  theme(axis.title = element_text(size=14),
        axis.text = element_text(size=14))
...
ggsave("Urchins-vs-kelp-coast-diff.png", width=5, heigh=3)

```

### Urchins at Tatoosh - mean across depths

```{r total-urchins-tatoosh, fig.width=5, fig.height=3}
  # Zoom in on urchins
urchin1 <- invert_transect %>% filter(grp2=="URCHIN") %>%
    group_by(year,site,transect,observer, area,grp2) %>%
    summarise(tot.density = sum(density, na.rm = TRUE)) %>% 
...
urchin_mean <- invert_transect %>% filter(grp2=="URCHIN") %>%
    ...
    summarise(mean.density=mean(tot.density, na.rm = TRUE), 
              ...
              N =length(year),
              SE= SD/sqrt(N) ) 
urchin_meant = urchin_mean %>% filter(site=="Tatoosh Island")  

ggplot() +
     # geom_point(data= urchin1,aes(x=year,y=mean.density,color=site)) +
      geom_point(data=urchin_meant,aes(x=year,y=mean.density)) +
      geom_line(data= urchin_meant,
                 aes(x=year,y=mean.density)) +
      geom_errorbar(data=urchin_meant,
                    aes(x=year,ymin=mean.density-SE,ymax=mean.density+SE),
                    width=0) +
      scale_x_continuous(breaks=seq(2015,last.year,2)) +
      scale_y_continuous(expression("Urchin density (m"^-2*")")) +
      # scale_color_viridis_d(option="plasma",end=0.75)+
...
### Urchins vs Nereocystis at Tatoosh

```{r urchins-kelp-tatoosh, fig.width=5, fig.height=3}

dat1 = algaex %>% 
  filter(site=="Tatoosh Island", species=="NERLUE") %>% 
  dplyr::select(year, mean.density) %>%
  rename(nero = mean.density)

dat2 = urchin_meant %>%
  filter(site=="Tatoosh Island",grp2=="URCHIN") %>% 
  dplyr::select(year, mean.density) %>%
  rename(urchins = mean.density)


dfx = left_join(dat1, dat2)

ggplot(dfx, aes(urchins, nero)) +
  xlab("Urchin density") + ylab("Nereo density") +
  annotate("text", x=dfx$urchins,y=dfx$nero, label=dfx$year) + 
  theme_nt + theme_bw()
...
```{r urchins-kelp-tatoosh-diff, fig.width=5, fig.height=3}

dnereo = diff(dfx$nero)
durchin = diff(dfx$urchins)
df1 = data.frame(cbind(dnereo, durchin))
df1$year = c(dfx$year[2]:2019, 2021:max(dfx$year))

ggplot(df1, aes(durchin,dnereo)) +
  # geom_point() + 
  xlab("Change in urchin density") + ylab("Change in Nereo density") +
  annotate("text", x=df1$durchin,y=df1$dnereo, label=df1$year) + 
  theme_nt + theme_bw() +
...
urchin.plot <- ggplot(urchin_year, aes(x = year, y=grand_mean, color=taxa)) +
                ...
                scale_x_continuous("", breaks=seq(2015,last.year,2)) +
                scale_y_continuous(expression("Urchins m"^-2), limits=c(0,5)) +
                theme_bw()+ theme_nt 
...
### Coastwide seastars by species x site x year

```{r total-seastars-year-5-10m, fig.height=3, fig.width=6}
...
#####
stars_year <- ggplot() +
      geom_point(data = star_zone, aes(x=year,y=grand_mean,color=species)) +
      geom_line( data = star_zone, aes(x=year,y=grand_mean,color=species)) +
      ...
      #                    labels = c(
      #                      bquote(italic('Henricia')),
      #                      bquote(italic('Leptasterias')),
      #                               'Large star', 
      #                      bquote(italic('Dermasterias')),
      #                      'Medium star',
...
stars_out = star_zone %>% 
  rename(index = grand_mean, SE = grand_SE, timeseries = species) %>%
  mutate(metric = "No per m2")

if(NT == TRUE){write.csv(stars_out, "~/GitHub/NMS-condition-reports/data/ocnms-dive-seastars.csv", row.names = F)}

```
### Subset of seastars blood, brood, leather

```{r total-seastars-year-subset, fig.height=3, fig.width=6}

x <-  invert_transect %>% filter(grp2=="STAR", 
                                 taxa %in% c("blood star", "brood star", "leather star")) %>%
      group_by(site,year,taxa) %>%
      summarise(Mean=mean(density, na.rm = TRUE),
                SD=sd(density),
                N=length(density),
...
### Subset of seastars blood, brood, leather, large

```{r total-seastars-year-subset2, fig.height=3, fig.width=6}

x <-  invert_transect %>% 
  filter(grp2=="STAR", taxa %in% c("blood star", "brood star", "leather star", "large star")) %>%
      group_by(site,year,taxa) %>%
      summarise(Mean=mean(density, na.rm = TRUE),
                SD=sd(density),
                N=length(density),
...
```{r total-seastars-year-subset-log, fig.height=3, fig.width=6}
...
stars_year <- ggplot() +
      ...
      scale_x_continuous(breaks=seq(2015,last.year,2)) +
      xlab("") +
      scale_y_continuous("Sea stars log index", trans='log10') + 
...
```{r spit-out-kelp-fish-4-multivariate, eval=FALSE}
...
kelp_wide$zone = as.numeric(kelp_wide$zone)
fish_wide$zone = as.numeric(fish_wide$zone)

fish_kelp_wide <- left_join( fish_wide, kelp_wide,                                               by=c('site'='site','year'='year','area'='area','zone'='zone'))
...
```{r tables-of-samples,  eval=FALSE}
...
fish_count_wide$zone = as.numeric(fish_count_wide$zone)
kelp_count_wide$zone = as.numeric(kelp_count_wide$zone)
...
```{r quick-kelp-plots, fig.width=6, fig.height=8,eval=FALSE}
...
```{r quick-fish-plots, fig.width=6.5, fig.height=8, eval=FALSE}
...
year.range = min(canopy$year):last.year
...
canopy.kelp <- ggplot( canopy %>% filter(., site == 'Coastwide') %>% filter(., year %in% year.range), 
                ...
                xlab("") +
               #scale_x_continuous(breaks=seq(2015,last.year,2)) +
                scale_x_continuous(breaks=seq(1990,last.year,5), minor_breaks = year.range) +
...
There is a discrepancy between the DNR data and kelpwatch data.  Need to check on it.

```{r kelpwatch, message=FALSE, error=FALSE}

kw = data.frame(read.csv(paste0(ocnms_dir, "/Data/Kelp/kelp-west-coast.csv")))
kw = kw[kw$region=="WA" & kw$quarter=="max",]


# need canopy kelp above

kelpwatch = canopy.kelp + 
  geom_line(data=kw, aes(year, hectares), color='Red', linewidth=1, alpha=0.5)  
