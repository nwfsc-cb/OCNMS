...
**open project for correct addressing**
...
# get just small rockfish
SPECIES = c("SEBYT", "SEME","SEFL", "SECA","SEMA",'RYOY','SEMY','SEPI','SENE', 'SEBSPP')
dat.yoy <- dat.fish %>% filter(species %in% SPECIES) %>%
              bind_rows(.,
                dat.fish %>% filter(size_class %in% c("small")))
# make complexes
dat.yoy$complex = NA
dat.yoy$complex[dat.yoy$species %in% c("SEBYT","SEME", "SEFL" )] <- 'YTB'
dat.yoy$complex[dat.yoy$species %in% c("SECA","SEMA")] <- 'CQB'
dat.yoy$complex[dat.yoy$species %in% c("SEPI")] <- 'SEPI' # canary
dat.yoy$complex[dat.yoy$species %in% c("SEMY")] <- 'SEMY' # blue or deacon
dat.yoy$complex[dat.yoy$species %in% c("SENE")] <- 'SENE' # china
dat.yoy$complex[dat.yoy$species %in% c("RYOY","SEBSPP")] <- 'RYOY' # unid
unique(dat.yoy$complex)
...
#| messange: false
#| warning: false
#| tbl-cap: "Number of transects conducted by year, site and depth zone.  Only transects that had at least 2-m visibility are included"
...
To calculate the average density of each rockfish complex in each year, we first calculate the mean density and standard error per site in each year. This approach means we are treating each transect as a i.i.d. sample of YOY density within each site and thus we ignore differences in abundance by depth zone.
...
```{r calc-yoy-ts, width=3.5, height=7}
#| echo: false
#| messange: false
#| warning: false
#| fig-cap: "Time-series of estimated young-of-year rockfish density on the Washington coast. Black points show means and standard errors for individual sites. Blue points show coastwide density estimates, interquartile range and 95\\% intervals for each year. Note y-axis is square root."

SPPz = unique(dat.yoy$complex)
SPPz
# GENERATE INDEXES BY SITE and AMONG SITES
dat.yoy.summary0 <- dat.yoy %>% filter(size_class=='small')

# dat.yoy.summary %>% distinct(species,year,size_class)
  
# summarize using means and standard errors
dat.yoy.summary <- dat.yoy.summary0 %>%
                    group_by(year,site, complex) %>% 
...
# loop for each spp group
for(i in 1:length(SPPz)){
  print(SPPz[i])
  dfx = dat.yoy.summary[dat.yoy.summary$complex == SPPz[i],]
  sim <- matrix(# simulate from a centered t-distribution.
        rt(nrow(dfx) * n.sim,dfx$N - 1) * dfx$SE +
                dfx$Mean, # Adjust to a different location and scale.
                nrow(dfx), n.sim) # Put it in a matrix.
  sim[sim <0] <- 0
  sim <- as.data.frame(sim)
  colnames(sim) <- paste0("X",1:ncol(sim))

  sim <- bind_cols(dfx %>% dplyr::select(year,site), sim) %>%
  ...
  yoy.trend$complex = SPPz[i]
  if(i==1){df_yoy = yoy.trend}else{df_yoy = rbind(df_yoy,yoy.trend)}  
}  
...
```{r plot-yoy-ts, width=3.5, height=7}
dfa = df_yoy
dfb = dat.yoy.summary
 
df_yoy$complex[df_yoy$complex=='YTB'] <- "Yellowtail & Black RF"
df_yoy$complex[df_yoy$complex=='SEPI'] <- "Canary RF"
df_yoy$complex[df_yoy$complex=='SEMY'] <- "Deacon RF"
df_yoy$complex[df_yoy$complex=='SENE'] <- "China RF"
df_yoy$complex[df_yoy$complex=='RYOY'] <- "Rockfish spp"
df_yoy$complex[df_yoy$complex=='CQB'] <- "Copper & Quillback RF"

dat.yoy.summary$complex[dat.yoy.summary$complex=='YTB'] <- "Yellowtail & Black RF"
dat.yoy.summary$complex[dat.yoy.summary$complex=='SEPI'] <- "Canary RF"
dat.yoy.summary$complex[dat.yoy.summary$complex=='SEMY'] <- "Deacon RF"
dat.yoy.summary$complex[dat.yoy.summary$complex=='SENE'] <- "China RF"
dat.yoy.summary$complex[dat.yoy.summary$complex=='RYOY'] <- "Rockfish spp"
dat.yoy.summary$complex[dat.yoy.summary$complex=='CQB'] <- "Copper & Quillback RF"

df_yoy$complex = factor(df_yoy$complex, levels = c('Yellowtail & Black RF','Canary RF','Deacon RF','Copper & Quillback RF','China RF','Rockfish spp')) 

dat.yoy.summary$complex = factor(dat.yoy.summary$complex, levels = c('Yellowtail & Black RF','Canary RF','Deacon RF','Copper & Quillback RF','China RF','Rockfish spp')) 

## Make yoy rockfish time-series plots
 yoy.ts <- ggplot() + 
   geom_point(data=dat.yoy.summary, aes(x=year,y=Mean),alpha=0.2,size=1) +
   geom_errorbar(data=dat.yoy.summary,
                 aes(x=year,ymin=Mean-SE,ymax=Mean+SE),
                 width=0,
                 alpha=0.2) +
   geom_point(data=df_yoy, aes(x=year,y=grand.mean),
              color="blue",size=1)  +
   geom_line(data=df_yoy, aes(x=year,y=grand.mean),
              color="blue")  +
   geom_ribbon(data=df_yoy, aes(year, ymin=q025, ymax=q975), 
               fill='blue',alpha=0.1) +
   ...
   xlab("Year") +
   facet_wrap(facets="complex", scale='free_y', ncol=1) + 
   #geom_hline(yintercept=0,color="red")+
   theme_bw()

yoy.ts

ggsave( paste0(fig_dir, "ocnms-rockfish-yoy-modeled.png"), units='in',
        width = 3.5, height = 7)
ggsave( "~/GitHub/ESR-rockfish-yoy/figures/ocnms-rockfish-yoy-modeled.png",
        units='in', width = 3.5, height = 7) 
  
# # save out ####
dat.yoy.sites = dat.yoy.summary

write.csv(df_yoy, "~/GitHub/ESR-rockfish-yoy/data/ocnms-rockfish-yoy.r")
write.csv(df_yoy, paste0(work_dir, "Estimated-YOY-trend-coast-", 
                            Sys.Date(),".csv"), row.names = F)
write.csv(dat.yoy.sites, paste0(work_dir, "Estimated-YOY-trend-sites-",
                                Sys.Date(),".csv"), row.names = F)
...
```{r fig-size-bin2, fig.height=8, fig.width=6.5}
#| echo: false
#| messange: false
#| warning: false
...
```{r fig-size-dist2, fig.height=8, fig.width=6.5}
#| echo: false
#| messange: false
#| warning: false
